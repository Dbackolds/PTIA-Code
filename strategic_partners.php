...
<!-- 战略合作伙伴管理 -->
<div id="strategic-partners-section" class="form-section">
    <h3>战略合作伙伴管理</h3>
    <div class="row mb-4">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="strategic-partners-title" value="<?php echo isset($strategicPartners['title']) ? htmlspecialchars($strategicPartners['title']) : ''; ?>">
                    </div>
                    <div class="mb-3">
                    ...
                    <div class="action-buttons">
                        <button id="save-strategic-partners" class="btn btn-primary">保存更改</button>
                    </div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', async function() {
    // 初始化战略合作伙伴管理
    const partnersTitle = document.getElementById('strategic-partners-title');
    const partnersContainer = document.getElementById('strategic-partners-container');
    const addButton = document.getElementById('add-strategic-partner');
    const saveButton = document.getElementById('save-strategic-partners');
    
    // 加载数据
    let strategicPartnersData = <?php echo json_encode($strategicPartners); ?>;
    ...
    saveButton.addEventListener('click', async function() {
        // 更新标题
        strategicPartnersData.title = partnersTitle.value;
        
        // 更新合作伙伴数据
        const partnerItems = document.querySelectorAll('.partner-item');
        strategicPartnersData.partners = Array.from(partnerItems).map((item, index) => {
            return {
