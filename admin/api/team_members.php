...
// 内容文件路径
$filePath = '../../data/team_members.json';
...
try {
    // 确保数据格式正确
    if (!isset($data['title']) || !isset($data['description']) || !isset($data['members']) || !is_array($data['members'])) {
        throw new Exception('数据格式不正确，缺少必要字段');
    }
    
    // 验证团队成员数据
    foreach ($data['members'] as $member) {
        if (!isset($member['name']) || !isset($member['position']) || !isset($member['avatar']) || !isset($member['description'])) {
            throw new Exception('团队成员数据格式不正确，缺少必要字段');
        }
    }
