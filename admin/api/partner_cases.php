...
// 内容文件路径
$filePath = '../../data/partner_cases.json';
$enFilePath = '../../data/partner_cases_en.json';
...
try {
    // 首先加载当前的中文和英文数据
    $currentZhData = file_exists($filePath) ? json_decode(file_get_contents($filePath), true) : ['title' => '合作案例', 'cases' => []];
    $currentEnData = file_exists($enFilePath) ? json_decode(file_get_contents($enFilePath), true) : ['title' => 'Partnership Cases', 'cases' => []];
    
    // 处理数据以过滤英文内容
    $partnerCasesData = json_decode($postData, true);
    $chineseCases = [];
    ...
    // 过滤案例数据，只保留中文案例
    foreach ($partnerCasesData['cases'] as $case) {
    ...
    if (empty($chineseCases)) {
        ...
        echo json_encode([
            'success' => true,
            'message' => '没有检测到新的中文合作案例数据，保持现有数据不变'
        ]);
        exit;
    } else {
        // 更新数据并保存
        $partnerCasesData['cases'] = $chineseCases;
        $result = file_put_contents($filePath, json_encode($partnerCasesData, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
    }
